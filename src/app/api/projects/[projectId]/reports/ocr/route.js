...
if (GOOGLE_AI_STUDIO_API_KEY) {
  try {
    ...
    console.error(
      "Failed to initialize Gemini components for OCR/PDR reports:",
      e
    );
  }
} else {
  console.warn(
    "GOOGLE_AI_STUDIO_API_KEY not set. Report functionality will be disabled."
  );
}
...
if (GOOGLE_CLOUD_PROJECT_ID && GCS_BUCKET_NAME) {
  try {
    ...
    console.log(
      `GCS Storage client initialized using keyfile ${keyFilePath} for bucket: ${GCS_BUCKET_NAME}`
    );
  } catch (e) {
  ...
  console.warn(
    "GCS_BUCKET_NAME or GOOGLE_CLOUD_PROJECT_ID not set. GCS download functionality will be disabled."
  );
}
...
export async function GET(request, { params }) {
  ...
  try {
    ...
    if (session && session.user && session.user.id) {
      ...
      isGuest = true;
      console.log(
        `OCR Report API: Guest access attempt with ID: ${effectiveGuestId} (Header: ${
    ...
    if (userId) {
      ...
      if (!project.guestOwnerId || project.guestOwnerId !== effectiveGuestId) {
        console.log(
          `OCR Report API: Guest ID mismatch: EffectiveID=${effectiveGuestId}, Project=${project.guestOwnerId}`
        );
      ...
      try {
        ...
        for (const diag of diagrams) {
          ...
          try {
            ...
            // --- End Explicit Check ---
            console.log(
              `OCR/PDR API - File: ${diag.fileName}, Detected MIME Type: ${detectedMimeType}`
            ...
            console.error(
              `OCR/PDR SSE: FATAL ERROR downloading GCS file ${objectPath} (${diag.fileName}):`,
...
Generate the PDR report in HTML format now, using both the OCR text and the provided diagram files for context.`;
          ...
          sendSseMessage(controller, {
            status: "Generating report summary ...",
          });
          ...
          const pdrResult = await generateContentWithRetry(
            ...
            {
              contents: [
                { role: "user", parts: [{ text: pdrPrompt }, ...fileParts] },
              ],
            ...
            (attempt, max) =>
              sendSseMessage(controller, {
                status: `Retrying report generation (${attempt}/${max})...`,
              }) // onRetry callback
          );
          ...
          if (!pdrReportHtml)
            throw new Error(
              "PDR generation process returned empty HTML after retries."
            ); // Updated error message
          sendSseMessage(controller, { status: "Report summary generated." });
        } catch (geminiError) {
          // Catch errors from either OCR or PDR generation
          console.error(
            "OCR/PDR SSE: Error during Gemini processing (OCR or PDR):",
            geminiError
          );
          ...
          if (geminiError.message && geminiError.message.includes("SAFETY")) {
            userFriendlyError =
              "Report generation could not be completed due to content safety guidelines.";
          } else if (
            geminiError.message &&
            ...
            userFriendlyError =
              "There was an issue processing one or more files for the report. Please check the file formats.";
          } else if (
            geminiError.message &&
            ...
            geminiError.message ===
            "PDR generation process returned empty HTML."
          ) {
            userFriendlyError =
              "Failed to generate the report summary based on the extracted text.";
          }
        ...
        if (pdrReportHtml.endsWith("```")) {
          pdrReportHtml = pdrReportHtml
            .substring(0, pdrReportHtml.length - 3)
            .trimEnd();
        }
        ...
        console.log(
          `OCR/PDR SSE stream complete for project ${projectId}. Sent public URL: ${pdfUrl}`
        );
      } catch (error) {
        // Main catch block for the stream start
        ...
        try {
          ...
          console.error(
            "OCR/PDR SSE Error: Failed to send error message to client:",
            sseError
          );
        }
      } finally {
        try {
          ...
          console.error(
            `SSE Error: Failed to close stream for project ${projectId}:`,
            e
          );
        }
      }
