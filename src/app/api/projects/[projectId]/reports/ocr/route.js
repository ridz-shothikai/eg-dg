    ...
    async start(controller) {
      ...
      try {
        ...
        for (const diag of diagrams) {
            ...
            tempFilePaths.push(tempFilePath); // Add path for potential error cleanup
            ...
            try {
                ...
                if (readError.code === 'ENOENT') {
                    console.warn(`SSE: File not found in local cache: ${tempFilePath}. Skipping.`);
                    sendSseMessage(controller, { status: `Skipping ${diag.fileName} (not cached)...` });
                    // Remove path if file wasn't actually read/created
                    const index = tempFilePaths.indexOf(tempFilePath);
                    if (index > -1) tempFilePaths.splice(index, 1);
                } else {
