export const MONGODB_URI = process.env.MONGODB_URI || 'mongodb+srv://admin:admin@cluster0.reeqo.mongodb.net/eg_dg';
export const GOOGLE_CLOUD_PROJECT_ID = process.env.GOOGLE_CLOUD_PROJECT_ID || 'shothikai-gcp';
export const GCS_BUCKET_NAME = process.env.GCS_BUCKET_NAME || 'temp-shothik';
export const GOOGLE_AI_STUDIO_API_KEY = process.env.GOOGLE_AI_STUDIO_API_KEY || 'AIzaSyDm4AuF7vILbSbwrT5hO0JlF3nq7UDozl0';
export const NEXTAUTH_SECRET = process.env.NEXTAUTH_SECRET || 'YOUR_NEXTAUTH_SECRET_PLACEHOLDER';
